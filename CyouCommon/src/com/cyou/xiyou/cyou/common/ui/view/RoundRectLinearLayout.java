...
public class RoundRectLinearLayout extends TouchableLinearLayout
{
    ...
    private void init(AttributeSet attrs)
    {
        if(attrs != null)
        {
            TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.RoundRectLinearLayout);

            try
            {
                this.cornerRadius = ta.getDimension(R.styleable.RoundRectLinearLayout_corner_radius, 0);
            }
            catch(Throwable t)
            {
                t.printStackTrace();
