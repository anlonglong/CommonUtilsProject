 ...
 */
public class RoundRectFrameLayout extends TouchableFrameLayout
{
    private Paint paint;
    ...
    private void init(AttributeSet attrs)
    {
        if(attrs != null)
        {
            TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.RoundRectFrameLayout);

            try
            {
                this.cornerRadius = ta.getDimension(R.styleable.RoundRectFrameLayout_corner_radius, 0);
            }
            catch(Throwable t)
            {
                t.printStackTrace();
