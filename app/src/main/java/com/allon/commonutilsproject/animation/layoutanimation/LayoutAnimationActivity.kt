package com.allon.commonutilsproject.animation.layoutanimation
...
class LayoutAnimationActivity : AppCompatActivity(), View.OnClickListener {
    ...
    companion object {
        fun start(ctx: Context) {
            ctx.startActivity(Intent(ctx, LayoutAnimationActivity::class.java))
        }
    }
    ...
    private fun changedAppending() {
        ...
        val animatorIn = PropertyValuesHolder.ofFloat("scaleX", 1f, 1.5f, 1f)
        val inAnim = ObjectAnimator.ofPropertyValuesHolder(
                container, pvhLeft, pvhRight, pvhTop, pvhBottom, animatorIn).setDuration(lt.getDuration(LayoutTransition.CHANGE_APPEARING))

        lt.setDuration(LayoutTransition.CHANGE_DISAPPEARING, 300)
        //translationX 移动的距离是相对于控件最初始的位置的移动距离。[这又是一个坑]
        val animatorOut = PropertyValuesHolder.ofFloat("translationX", 0f, 100f, -100f, 50f, -50f,0f)
        val outAnim = ObjectAnimator.ofPropertyValuesHolder(
                container, pvhLeft, pvhRight, pvhTop, pvhBottom, animatorOut).setDuration(lt.getDuration(LayoutTransition.CHANGE_DISAPPEARING))


        lt.setAnimator(LayoutTransition.CHANGE_APPEARING, inAnim)
        lt.setAnimator(LayoutTransition.CHANGE_DISAPPEARING, outAnim)
